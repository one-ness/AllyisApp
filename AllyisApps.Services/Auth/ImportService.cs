				...
				foreach (DataRow row in table.Rows)
				{
					...
					if (hasUserEmail || hasEmployeeId || hasUserName)
					{
						...
						if (userInOrg != null && hasNonRequiredUserInfo)//If user exists then allow org to change user.
						{
							...
							if (updated)
							{
								await UpdateUserProfile(userInOrg.UserId, userInOrg.DateOfBirth, userInOrg.FirstName, userInOrg.LastName, userInOrg.PhoneNumber, userInOrg.Address?.Address1, userInOrg.Address?.City, userInOrg.Address?.StateId, userInOrg.Address?.PostalCode, userInOrg.Address?.CountryCode);
							}
						}
					}
				}
