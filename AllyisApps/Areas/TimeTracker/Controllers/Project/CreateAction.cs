		...
		public int CreateProjectAndUpdateItsUserList(EditProjectViewModel model)
		{
			...
			return AppService.CreateProjectAndUpdateItsUserList(
				new Services.Project.Project()
				{
					...
					StartingDate = model.StartDate == 0 ? null : Utility.GetDateTimeFromDays(model.StartDate),
					EndingDate = model.StartDate == 0 ? null : Utility.GetDateTimeFromDays(model.EndDate)
				},
				userIds);
		}
