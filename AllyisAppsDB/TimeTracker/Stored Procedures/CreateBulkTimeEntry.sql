...
BEGIN
	SET NOCOUNT ON;
	DECLARE @tTProductId INT = (SELECT [ProductId] FROM [Billing].[Product] WHERE [Product].ProductId = 200000);
	...
	set @firstProject = (SELECT TOP 1 [ProjectId] 
		FROM [Pjm].[Project] WITH (NOLOCK) 
				JOIN [Crm].[Customer] WITH (NOLOCK) 
				ON [Customer].[CustomerId] = [Project].[CustomerId] WHERE [Customer].[OrganizationId] = @organizationId AND Project.IsDefault = 1);
	...
	INSERT INTO [TimeTracker].[TimeEntry] ([UserId], [Date], [Duration], [Description], [PayClassId], [ProjectId],[TimeEntryStatusId])
	SELECT [UserId], [Date], [Duration], [Description], [PayClassId], [FirstProject] AS 'ProjectId',0  FROM #OrgTmp;
	DROP TABLE #OrgTmp;
	DROP TABLE #SKUIDs;
END