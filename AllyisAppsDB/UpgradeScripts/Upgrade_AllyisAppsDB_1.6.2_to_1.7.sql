...
GO
/*
The column [StaffingManager].[StaffingSettings].[OrganizationId] is being dropped, data loss could occur.
*/
/*
IF EXISTS (select top 1 1 from [StaffingManager].[StaffingSettings])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT
...
GO
*/
DELETE TimeTracker.TimeEntry WHERE ProjectId =0;
...
GO
PRINT N'Dropping unnamed constraint on [StaffingManager].[StaffingSettings]...';


GO
ALTER TABLE [StaffingManager].[StaffingSettings] DROP CONSTRAINT [DF__StaffingS__Organ__7C6F7215];
...
GO
PRINT N'Dropping [StaffingManager].[FK_Organization]...';


GO
ALTER TABLE [StaffingManager].[StaffingSettings] DROP CONSTRAINT [FK_Organization];


GO
PRINT N'Dropping [StaffingManager].[FK_DefaultStatus]...';


GO
ALTER TABLE [StaffingManager].[StaffingSettings] DROP CONSTRAINT [FK_DefaultStatus];
...
GO
PRINT N'Starting rebuilding table [StaffingManager].[StaffingSettings]...';
...
CREATE TABLE [StaffingManager].[tmp_ms_xx_StaffingSettings] (
    ...
    CONSTRAINT [tmp_ms_xx_constraint_PK_StaffingSettingsId1] PRIMARY KEY CLUSTERED ([StaffingSettingsId] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [StaffingManager].[StaffingSettings])
    BEGIN
        SET IDENTITY_INSERT [StaffingManager].[tmp_ms_xx_StaffingSettings] ON;
        INSERT INTO [StaffingManager].[tmp_ms_xx_StaffingSettings] ([StaffingSettingsId], [DefaultPositionStatusId])
        SELECT   [StaffingSettingsId],
                 [DefaultPositionStatusId]
        FROM     [StaffingManager].[StaffingSettings]
        ORDER BY [StaffingSettingsId] ASC;
        SET IDENTITY_INSERT [StaffingManager].[tmp_ms_xx_StaffingSettings] OFF;
    END

DROP TABLE [StaffingManager].[StaffingSettings];

EXECUTE sp_rename N'[StaffingManager].[tmp_ms_xx_StaffingSettings]', N'StaffingSettings';

EXECUTE sp_rename N'[StaffingManager].[tmp_ms_xx_constraint_PK_StaffingSettingsId1]', N'PK_StaffingSettingsId', N'OBJECT';
...
ALTER TABLE [TimeTracker].[Setting]
    ADD [PayrollProcessedDate] DATE          NULL,
        [LockDate]             DATE          NULL,
        [PayPeriod]            VARCHAR (MAX) DEFAULT '{"type":"Duration","duration":"14","startDate":"2017/10/16"}' NULL;
...
GO
PRINT N'Creating [StaffingManager].[FK_Subscription]...';


GO
ALTER TABLE [StaffingManager].[StaffingSettings] WITH NOCHECK
    ADD CONSTRAINT [FK_Subscription] FOREIGN KEY ([SubscriptionId]) REFERENCES [Billing].[Subscription] ([SubscriptionId]);
...
END
GO
PRINT N'Altering [StaffingManager].[CreateStaffingSettings]...';


GO
ALTER PROCEDURE [StaffingManager].[CreateStaffingSettings]
	@organizationId			 INT,
	@subscriptionId			 INT,
...
END
GO
PRINT N'Altering [StaffingManager].[UpdateStaffingSettings]...';


GO
ALTER PROCEDURE [StaffingManager].[UpdateStaffingSettings]
	@subscriptionId INT,
...
AS
	...
	UPDATE [TimeTracker].[Setting]
		...
		WHERE [OrganizationId] = @organizationId;
GO
PRINT N'Creating [TimeTracker].[UpdatePayPeriod]...';
...
ALTER TABLE [StaffingManager].[StaffingSettings] WITH CHECK CHECK CONSTRAINT [FK_Subscription];
